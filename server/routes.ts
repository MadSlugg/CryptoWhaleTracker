...
class OrderGenerator {
  ...
  private async fetchWhaleOrders(exchangeId: Exclude<Exchange, 'all'>) {
    ...
    try {
      ...
      const whaleOrders = await config.service.getWhaleOrders(8400000, validReferencePrice);
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/filled-order-analysis", async (req, res) => {
    try {
      ...
      // Get minimum size filter (default 100 BTC to match whale threshold)
      let minSize = 100;
      if (req.query.minSize) {
        const parsed = parseFloat(req.query.minSize as string);
        if (isNaN(parsed) || parsed < 0) {
  ...
  // API endpoint for confidence-based entry point recommendations
  app.get("/api/entry-points", handleEntryPoints);
